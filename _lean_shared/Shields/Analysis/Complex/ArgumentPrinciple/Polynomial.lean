...
/-- A pole strictly outside the closed disc contributes nothing: `(z - w)⁻¹` is differentiable on
the closed disc and Cauchy's theorem applies. -/
theorem circleIntegral_sub_inv_of_lt {c w : ℂ} {R : ℝ} (hR : 0 ≤ R) (hw : R < dist w c) :
    (∮ z in C(c, R), (z - w)⁻¹) = 0 := by
  have hne : ∀ z ∈ closedBall c R, z - w ≠ 0 := by
    intro z hz hzw
    rw [sub_eq_zero] at hzw
    rw [mem_closedBall, hzw] at hz
    exact absurd hz (not_le.mpr hw)
  ...
  · intro z hz
    exact (differentiableAt_id.sub_const w).inv (hne z (ball_subset_closedBall hz.1))
...
-- integral, so the hypothesis is weakened to joint continuity of `(t, θ) ↦ F t (circleMap c R θ)`.
/-- **The count moves continuously.**  A circle integral of a family that is jointly continuous
along the circle is continuous in the parameter. -/
theorem continuous_circleIntegral_param {c : ℂ} {R : ℝ} (F : ℝ → ℂ → ℂ)
    (hF : Continuous fun p : ℝ × ℝ => F p.1 (circleMap c R p.2)) :
    Continuous fun t : ℝ => ∮ z in C(c, R), F t z := by
  simp only [circleIntegral]
  apply intervalIntegral.continuous_parametric_intervalIntegral_of_continuous'
  have h1 : Continuous fun p : ℝ × ℝ => deriv (circleMap c R) p.2 := by
...
theorem circleIntegrable_multiset_sum_sub_inv {c : ℂ} {R : ℝ} (s : Multiset ℂ)
  ...
  | empty => simp
  | cons w s ih =>
    have h1 : CircleIntegrable (fun z => (z - w)⁻¹) c R :=
      circleIntegrable_sub_inv_iff.2 (Or.inr (hs w (Multiset.mem_cons_self w s)))
    have h2 := ih fun r hr => hs r (Multiset.mem_cons_of_mem hr)
    have h3 : CircleIntegrable
        (fun z => (z - w)⁻¹ + (s.map fun r => (z - r)⁻¹).sum) c R := h1.add h2
    simpa [Multiset.map_cons, Multiset.sum_cons] using h3
...
`∮ ∑_{r ∈ s} (z - r)⁻¹ = 2πi · #{r ∈ s : |r - c| < R}`, the count taken with multiplicity. -/
theorem circleIntegral_multiset_sum_sub_inv {c : ℂ} {R : ℝ} (hR : 0 < R) (s : Multiset ℂ)
  ...
  | cons w s ih =>
    ...
    have h1 : CircleIntegrable (fun z => (z - w)⁻¹) c R :=
      circleIntegrable_sub_inv_iff.2 (Or.inr (by rwa [habs]))
    have h2 : CircleIntegrable (fun z => (s.map fun r => (z - r)⁻¹).sum) c R :=
    ...
    · rw [circleIntegral.integral_sub_inv_of_mem_ball (mem_ball.mpr hb),
        Multiset.filter_cons_of_pos (p := fun r => dist r c < R) s hb, Multiset.card_cons]
      push_cast
      ring
    · have hlt : R < dist w c := by
        ...
        · exact h
      rw [circleIntegral_sub_inv_of_lt hR.le hlt,
...
theorem card_rootsIn_add_eq {P Q : Polynomial ℂ} {c : ℂ} {R : ℝ} (hR : 0 < R)
  ...
  have habs : |R| = R := abs_of_pos hR
  -- (a) The family stays zero-free on the circle.
  ...
  have hP0 : ∀ t : ℝ, rouchePoly P Q t ≠ 0 := by
    intro t h0
    exact hne t _ hsph (by rw [h0]; simp)
  have hns : ∀ (t : ℝ), ∀ r ∈ (rouchePoly P Q t).roots, r ∉ sphere c R := by
    intro t r hr hmem
    exact hne t r hmem (mem_roots'.mp hr).2
  -- Each count is an integer, and equals the contour integral over `2πi`.
  set N : ℝ → ℤ := fun t => ((rootsIn (rouchePoly P Q t) c R).card : ℤ) with hNdef
  have hcount : ∀ t : ℝ,
      (∮ z in C(c, R), (derivative (rouchePoly P Q t)).eval z / (rouchePoly P Q t).eval z)
  ...
  have hjoint : Continuous fun p : ℝ × ℝ =>
      (derivative (rouchePoly P Q p.1)).eval (circleMap c R p.2)
        / (rouchePoly P Q p.1).eval (circleMap c R p.2) := by
    simp only [eval_rouchePoly, derivative_eval_rouchePoly]
    refine Continuous.div ?_ ?_ ?_
    · exact ((derivative P).continuous.comp hcm).add
        (hcl.mul ((derivative Q).continuous.comp hcm))
    · exact (P.continuous.comp hcm).add (hcl.mul (Q.continuous.comp hcm))
    · intro p
      have h := hne p.1 (circleMap c R p.2) (circleMap_mem_sphere c hR.le p.2)
      rwa [eval_rouchePoly] at h
...
The `inf > 0` step is compactness plus continuity: `z ↦ ‖P₀.eval z‖` is continuous and the sphere
is compact and nonempty, so the infimum is attained, and it is positive because `P₀` has no zero
there. -/
theorem eventually_card_rootsIn_eq {c : ℂ} {R : ℝ} (hR : 0 < R) {P : ℕ → Polynomial ℂ}
  ...
  have hsph : (c + (R : ℂ)) ∈ sphere c R := by
    rw [mem_sphere, dist_eq_norm, add_sub_cancel_left, Complex.norm_real, Real.norm_eq_abs, habs]
  obtain ⟨z₀, hz₀, hmin⟩ := (isCompact_sphere c R).exists_isMinOn ⟨_, hsph⟩
    (P₀.continuous.norm.continuousOn)
  have hεpos : 0 < ‖P₀.eval z₀‖ := norm_pos_iff.mpr (hns z₀ hz₀)
  have hbound : ∀ z ∈ sphere c R, ‖P₀.eval z₀‖ ≤ ‖P₀.eval z‖ := fun z hz =>
    isMinOn_iff.mp hmin z hz
  filter_upwards [Metric.tendstoUniformlyOn_iff.mp hunif _ hεpos] with n hn
  have hlt : ∀ z ∈ sphere c R, ‖(P n - P₀).eval z‖ < ‖P₀.eval z‖ := by
    intro z hz
    rw [eval_sub]
    calc ‖(P n).eval z - P₀.eval z‖ = dist (P₀.eval z) ((P n).eval z) := by
          rw [dist_eq_norm, norm_sub_rev]
      _ < ‖P₀.eval z₀‖ := hn z hz
      _ ≤ ‖P₀.eval z‖ := hbound z hz
