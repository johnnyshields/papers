...
/-
VENDORED FROM AN EXTERNAL LEAN PROJECT -- see `Vendor/HexRealRootsMathlib.lean` for the
full quality audit, the licence, and the retirement condition.
* Project: hex-real-roots-mathlib, part of the `hex` computer algebra library,
  <https://github.com/leanprover/hex-real-roots-mathlib>
* Upstream path: `HexRealRootsMathlib/TwoCircleSector.lean`
* Revision: `30c19a426b3c195e067f2c15d5a6ca06eff7df66`
* Licence: Apache 2.0.  The copyright and `Authors:` line above are upstream's.
This file is taken byte-identically: it imports `Mathlib` and nothing else, so no
import rewriting was needed and no proof was adapted.
-/
