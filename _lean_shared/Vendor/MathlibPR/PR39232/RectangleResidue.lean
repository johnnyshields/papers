...
* **Source PR:** `feat(Analysis/Complex): residue theorem for rectangular
  contours` --
  <https://github.com/leanprover-community/mathlib4/pull/39232>
* **PR author:** Jeremy Tan (GitHub `jerwaynejones`).  No co-authors.
* **Context:** Mathlib carries Cauchy--Goursat for rectangles
  (`Complex.integral_boundary_rect_eq_zero_of_differentiable_on_off_countable`)
  and the Cauchy integral formula for circles, but no residue theorem for a
  *rectangular* contour.  Upstream fills that gap; the PR was extracted from a
  separate formalization where Perron's formula needs contour shifting on
  rectangles.
* **Licence:** Apache 2.0, as all of Mathlib.  The copyright and `Authors:` line
  above are upstream's and are preserved deliberately.

Upstream adds one new file and one `Mathlib.lean` import line.  The new file is
reproduced below unchanged apart from its module docstring, which is this one;
declarations keep their upstream namespace `Complex` and their upstream names,
so consuming code is written exactly as it will be against a merged Mathlib.

## What this supplies

* `Complex.boundaryIntegral` -- the oriented boundary integral over the
  rectangle `[z, w]`, in the sign convention of Mathlib's Cauchy--Goursat.
* `Complex.log_neg` -- the principal logarithm rotated so that its cut lies on
  the *positive* real axis, with `Complex.hasDerivAt_log_neg`.  The left edge of
  a rectangle straddling the origin crosses the standard cut, which is why a
  second branch is needed at all.
* `Complex.boundaryIntegral_inv_sub_eq_two_pi_I` -- the closed form
  `∮_{∂[z,w]} (s - ρ)⁻¹ ds = 2πi` for `ρ` strictly inside.
* `Complex.boundaryIntegral_eq_residue_sum` and
  `Complex.boundaryIntegral_single_pole` -- the residue theorem for a finite sum
  of **simple** poles plus a holomorphic remainder, bundled as
  `Complex.RectangleResidueData`.

Poles of higher order are **not** covered upstream and are not added here.

## When the PR merges

Bump the Mathlib pin, delete this file, and replace `import
Vendor.MathlibPR.PR39232.RectangleResidue` with `import Mathlib.Analysis.Complex.RectangleResidue`
at each use site.  Nothing else changes: the names are already upstream's.

## Adaptations to the pinned Mathlib

One, marked inline: `Complex.hasDerivAt_clog_neg_real` is closed by rewriting the
derivative rather than by upstream's `simpa`, whose normalization of the
composite crosses between the two `Module ℝ ℂ` instance paths (`NormedAlgebra`
and `InnerProductSpace`) and fails to unify at reducible transparency.  The
...
theorem hasDerivAt_clog_neg_real {f : ℝ → ℂ} {x : ℝ} {f' : ℂ}
  ...
  -- Adapted to the pin: upstream closes this with `simpa`, which here normalizes the
  -- composite to `fun t => (f t).log_neg` and lands on the `InnerProductSpace ℝ ℂ`
  -- rather than the `NormedAlgebra ℝ ℂ` path to `Module ℝ ℂ`.  Rewriting the
  -- derivative instead of the function keeps one instance path throughout.
  have h := (hasDerivAt_log_neg h₂).comp x h₁
  simp only [Function.comp_def] at h
  rw [div_eq_mul_inv, mul_comm f' ((f x)⁻¹)]
  exact h
