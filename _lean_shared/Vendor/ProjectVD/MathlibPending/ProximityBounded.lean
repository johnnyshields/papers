/-
Vendored from ProjectVD, `VD/MathlibPending/ProximityBounded.lean`, by Stefan Kebekus (GitHub `kebekus`).
...
-/
import Vendor.ProjectVD.MathlibPending.PoissonJensen
...
lemma proximity_isBigO_log_of_polynomial (p : Polynomial ℂ) :
  ...
  calc log⁺ ‖p.eval x‖
      ≤ log⁺ (C * r ^ p.natDegree) :=
        ...
        posLog_le_posLog ((norm_nonneg _))
