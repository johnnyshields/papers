/-
Vendored from ProjectVD, `VD/MathlibSubmitted/CountingEstimate.lean`, by Stefan Kebekus (GitHub `kebekus`).
...
is being upstreamed to Mathlib piece by piece.  It is submitted upstream as Mathlib pull request #41864, `feat: estimate for logarithmic counting functions`,
  https://github.com/leanprover-community/mathlib4/pull/41864
...
-- Adapted to the pinned revision: `ite_eq_left` / `ite_eq_right` postdate the pin; the pinned
-- core states the same rewrites as `if_pos` / `if_neg`.
