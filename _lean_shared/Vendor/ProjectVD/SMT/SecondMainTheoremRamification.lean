/-
Vendored from ProjectVD, `VD/SMT/SecondMainTheoremRamification.lean`, by Stefan Kebekus (GitHub `kebekus`).
...
-/
import Vendor.ProjectVD.SMT.DivisorDeriv
import Vendor.ProjectVD.SMT.ProximityEstimates
    ...
    have hS : proximity (logDeriv f) ⊤ r + ∑ a ∈ s, proximity (logDeriv (f · - a)) ⊤ r
      ...
      calc proximity (logDeriv f) ⊤ r + ∑ a ∈ s, proximity (logDeriv (f · - a)) ⊤ r
        ...
        _ ≤ C₀ * ‖log⁺ (characteristic f ⊤ r) + Real.log r‖ := by
          -- Adapted to the pinned revision, where `Asymptotics.IsBigO.sum` concludes about
          -- `fun x ↦ ∑ i ∈ s, A i x` rather than the `Finset` sum of functions, so `hbound` is
          -- already in the pointwise form of the goal and needs no `Finset.sum_apply` rewrite.
          exact hbound
        _ = C₀ * (log⁺ (characteristic f ⊤ r) + Real.log r) := by
