#!/usr/bin/env python3
"""Paper section `subsec:gram` (Gram structure and the exceptional matrix M_1), subsec:gram:
lem:M1-indefinite and eq:det-M1.
...
and emits the pgfplots coordinate block that the figure inlines:
  ...
  * the exact value f(1/2) = pi^2/2 - 4 > 0, which is the right-hand end of the
    bracket the lemma's proof uses, together with f(1/4) = -4 as its left end;
  ...
  * the threshold a_* = 0.3690738484..., and the sign of f on either side of it,
    including the ten-digit literal fig:defect-localization prints in panel (a);
  * the inertia trichotomy of lem:M1-indefinite read off the eigenvalues of M_1,
    which is the classification that figure's panel (a) states.

Numerical work is mpmath at arbitrary precision.
"""
...
# --- exact values at the ends of the lemma's bracket ----------------------
...
print('PASS: f(1/4) = -4 < 0 < f(1/2) = pi^2/2 - 4 = '
      f'{mp.nstr(f(A_HI), 12)}, det M_1(1/4) = -16 -- the sign change is bracketed')
...
assert mp.nstr(a_star, 10) == '0.3690738484', mp.nstr(a_star, 20)   # lem:M1-indefinite
...
# Panel (a) of fig:defect-localization prints a_* to ten places, and the paper
# quotes the same ten in lem:M1-indefinite.  Pin that literal to the curve.
lit = mp.mpf('0.3690738484')
assert abs(f(lit)) < mp.mpf('1e-7'), mp.nstr(f(lit), 8)
print('PASS: the a_* literal 0.3690738484 printed by the paper and by panel (a) '
      'sits on the curve')
...
# monotone on (0,infty), so lem:M1-indefinite cannot get uniqueness of a_* from monotonicity
# alone and must route a >= 1/2 through thm:gram.
...
# --- a monotone sweep across the bracket, independent of the series bound ---
...
for (a0, y0), (a1, y1) in zip(samples, samples[1:]):
    assert y1 > y0, (a0, a1)
print(f'PASS: {N_SAMPLES} samples across [1/4, 1/2] strictly increasing, with the '
      'sign change exactly at a_*')
