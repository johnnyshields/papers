#!/usr/bin/env python3
"""Paper section `sec:main` (Reciprocal-gamma formulation and positivity phase diagram, sec:main) and section `subsec:finite-defect`
(Finite-defect localization, subsec:finite-defect).

On the slice tau = 1 the constant term vanishes for the whole family, so the sign
of the degree-one coefficient is what detects the threshold in kappa uniformly in
a -- the observation rem:fiber-deformation records and thm:two-parameter-coeff turns into
the exact boundary tau_cw.  What the degree-one coefficient alone supports is the
set identity

    { kappa : Delta_1^(kappa)(a) > 0 for every a > 0 }  =  [1, infinity).

Delta_1^(kappa) is not displayed in the paper; it is eq. (Delta1-sharp) with M_1 replaced by
M_1^(kappa), i.e. eq. (MD01-kappa) scaled by S_0 S_1:

    Delta_1^(kappa)(a) = 2/(a^3 Gamma(a)^4) * [ (kappa-1)/2 (a g)^2 + a g - 1 ],
    g = psi_1(a).

Checked here:
  (1) a g - 1 > 1/(2a) for a > 0                  (eq. (trig-lower), at y = a)
...
than prove it for every a; the unconditional statement is eq. (Delta1-sharp) plus (1), and the
...
# (1) the lower bound a*psi_1(a) - 1 > 1/(2a), i.e. eq. (trig-lower) at y = a, which the
#     display closing sec:endpoint uses
for a in LADDER:
    lhs = a * psi(1, a) - 1
...
# eq. (MD01-kappa) derived from the coefficient matrices, not restated.  Building
# MD(M_0, M_1^(kappa)) from the entries of sec:coefficients and comparing with the bracket
# is what makes the sign test above a test of the paper rather than of itself.
#   M_0 = [[g, sqrt g],[sqrt g, 1]]  (the display after thm:gram),
...
print("OK    eq. (MD01-kappa) MD(M_0,M_1^(kappa)) rebuilt from the sec:coefficients entries,")
print("      and Delta_1^(kappa) = S_0 S_1 MD(M_0,M_1^(kappa)); kappa = 1 is eq. (Delta1-sharp)")
...
print("eq. (Delta1-sharp) and eq. (trig-lower).  The degree-one sign does not by itself yield positivity")
print("of the higher coefficients -- that is the endpoint argument of subsec:finite-defect.")
print("ALL PASS")
