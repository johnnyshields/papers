...
\cite[\S5.9 and eq.~5.15.1]{NISTDLMFGamma}.
...
\begin{remark}[Genuine indefiniteness of \texorpdfstring{\(M_1\)}{M1}]\label{rem:M1-indefinite}
The anomaly at $m=1$ is genuine.  Set $f(a)=(4a-1)\psi_1(a)-4$, so that $\det M_1$ has the sign of $f(a)$ by \eqref{eq:det-M1}.  Since $\psi_1(a)=\sum_{r\ge0}(a+r)^{-2}$, for $0<a<1/2$
\[
 f'(a)=2\sum_{r\ge0}\frac{2r+1-2a}{(a+r)^3}>0,
\]
while $f(a)\to-\infty$ as $a\downarrow0$ and $f(1/2)=\psi_1(1/2)-4=\pi^2/2-4>0$.  Hence $f$ has a unique zero $a_*\in(0,1/2)$.  With \Cref{thm:gram} for $a\ge1/2$ and the positive diagonal of $M_1$, the matrix $M_1$ is indefinite for $0<a<a_*$, positive semidefinite of rank one at $a=a_*$, and positive definite for $a>a_*$.  It is the only coefficient matrix that can be indefinite at the endpoint $\kappa=1$, so the determinant positivity of \Cref{thm:coefficientwise} cannot follow from pointwise semidefiniteness of the $M_m$ alone.
\end{remark}
...
\end{equation}
For $a\ge1/2$, $M_0$ is rank-one positive semidefinite and $M_m\succ0$ for
$m\ge1$ by \Cref{thm:gram}, so \Cref{lem:MD-positive} makes every summand of
\eqref{eq:Delta-n-MD} nonnegative and the pair $(M_0,M_n)$ strictly positive,
giving $\Delta_n>0$; the argument that follows is uniform in $a>0$.
...
\begin{equation}\label{eq:M1-Mm-positive}
 \MD(M_1,M_m)
 =\alpha_1(1+gc_m)+\alpha_m-2g\beta_1\beta_m\ge0.
\end{equation}
If $a\ge1/2$ then $M_1,M_m\succeq0$ by \Cref{thm:gram} and the nonstrict case of
...
Hence $\Delta_n>0$.  The self-pairing $\MD(M_1,M_1)=2\det M_1$, excluded by this
classification, occurs solely in $\Delta_2$; by \Cref{rem:M1-indefinite} the
indefiniteness of $M_1$ for small $a>0$ can contribute negatively only there.
...
\end{equation}
holds and shows that $g=\psi_1(a)>0$ \cite[eq.~5.15.1]{NISTDLMFGamma}.  Hence the positive real
...
\cite{KalmykovKarp2013Ratios,KalmykovKarp2014}.  The sharp coefficientwise thresholds of Karp and Zhang \cite[\S3]{KarpZhang2024} are set by the multiplicity of Pochhammer symbols in such scalar series; the threshold here is instead the deformation parameter $\kappa$ of a coupled matrix determinant.
...
order \cite[\S\S2.2, 3]{BariczPogany2014} have also been studied, as has the monotonicity in
...
argument curvature.  First-order derivatives of $I_\nu$ with respect to the order, which underlie
$G_\nu$ and $P_\nu$, have been studied in their own right, with closed forms
and integral representations \cite[\S10.38]{NISTDLMFOrder},\,\cite{GonzalezSantander2018}; those
results represent the separate
...
totally positive for $x>0$ and $s\ge0$ \cite{Salazar2026Darboux}, extending
integer-order results of Buchstaber and Glutsyuk \cite[Thm.~1.4]{BuchstaberGlutsyuk2019};
...
\smallskip
\noindent\textbf{Declaration of generative AI and AI-assisted technologies in the manuscript preparation process.} During the preparation of this work, the author used OpenAI's ChatGPT (GPT-5.6 Sol) and Anthropic's Claude (Claude Opus 4.8) to assist with manuscript drafting and the development of auxiliary verification code.  After using these tools, the author reviewed and edited the content as needed and takes full responsibility for the content of the publication.
