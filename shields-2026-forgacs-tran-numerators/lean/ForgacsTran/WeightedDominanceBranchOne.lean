...
The corner is closed at every `3 ≤ n`.  `(deg Q, r) = (2,1)` is admissible and is
settled by a different route: `τ` is constant there, so the separation results the
dominance argument runs on exclude it, and `rem:quadratic-case` reaches the sharper
all-index bound `deg B + 2` by quasi-orthogonality instead.  That exclusion is
carried here by the lower block and the interior supply; the upper block holds at
`2 ≤ n` and is not what bounds the reach.
...
**No condition is placed on the numerator at the collision, and the exponent is
what carries the difference.**  `p_1 = ord_{-L}(B) - 1`, which is
`lem:amplitude-divisor`'s `p = \nu - (k-1)` at the `k = 2` of an `r = 1` finite
upper endpoint.  At `ord \le 1` that is `0` and the floor is a constant — the
amplitude diverges at `ord = 0` and tends to a positive limit at `ord = 1` — and at
`ord \ge 2` the amplitude vanishes and the floor decays at the honest rate.
`B(0) \ne 0` is still required, by the consumer, and is a separate condition: `0`
and `-L` are different points.
...
`2L`, and both members are simple.  `τ ≤ 3L/2` against that circle gives
`σ_1 = 3/4`, and the amplitude floor runs at `p_1 = ord_{-L}(B) - 1` — the exponent
needs `L`, which is why the numerator is quantified after it.
...
theorem exists_upper_endpoint_block_one {n : ℕ} {a : Fin n → ℝ} {c x₁ : ℝ}
      ...
      ∀ B : Polynomial ℂ, B ≠ 0 →
        ∃ C₁ A₁ : ℝ, 0 ≤ C₁ ∧ 0 < A₁ ∧ ∃ e₁ > (0 : ℝ), e₁ < π ∧
          ∀ δ : ℝ, 0 < δ → δ ≤ e₁ →
            ...
            ∧ A₁ * δ ^ (B.rootMultiplicity ((-L : ℝ) : ℂ) - 1)
  ...
  obtain ⟨A₁, hA₁, eA, heA, hA⟩ :=
    ftPrincipalAmp_floor_of_endpoint_pi_any_multiplicity (x₁ := x₁) (B := B)
      hn2 ha hc hL hE hτ hz hB0
...
**Nothing is asked of the numerator at the collision.**  `thm:weighted-dominance`
asks nothing there either, and `lem:amplitude-divisor` covers a numerator vanishing
at a finite endpoint to arbitrary order; the exponent `p_1 = ord_{-L}(B) - 1` is
what absorbs it.
...
the exponent `ord_{-L}(B) - 1` cannot even be stated until it is fixed.
...
**`3 ≤ n` is where the paper's own route changes, not a limit of the
formalization.**  `(deg Q, r) = (2,1)` — exactly `n = 2` here — is admissible, but
`τ` is constant on that pencil, so the separation results this argument runs on
exclude it and `rem:quadratic-case` settles it by quasi-orthogonality, with a bound
sharper than the one reached here.  Both the lower endpoint block and the interior
supply carry the exclusion as a hypothesis.  The upper endpoint asks less —
`exists_upper_endpoint_block_one` holds at every `n ≥ 2` — so nothing on this side
is what bounds the reach.
...
theorem ft_weighted_dominance_one {n ρ : ℕ} {a : Fin n → ℝ} {c x₁ : ℝ}
  ...
  refine ⟨t₀, ht₀, L, hL, fun B hB hB0 => ?_⟩
  ...
  obtain ⟨C₁, A₁, hC₁, hA₁, e₁b, he₁b, -, hup⟩ := hblk B hB0'
  ...
  exact weighted_dominance_of_branch_any_multiplicity_at_of_threshold_of_data
      ...
      (n₁ := 0) (p₁ := B.rootMultiplicity ((-L : ℝ) : ℂ) - 1)
      (τmax₁ := 3 * L / 2) (σ₁ := 3 / 4) (R₁ := 2 * L)
      (L₁ := fun i => i.elim0) (c₁ := 0)
...
multiplicity, so what is left is the admissible class and `B(0) ≠ 0`.
...
theorem ft_weighted_dominance_one_unconditional {n ρ : ℕ} {a : Fin n → ℝ} {c x₁ : ℝ}
  ...
  refine ⟨h, hh, L, hL, fun B hB hB0 => ?_⟩
  have hB0' : B ≠ 0 := fun h0 => hB0 (by rw [h0]; simp)
...
`B = 1` meets `B(0) ≠ 0` whatever collision point the upper endpoint hands back.
What the corner no longer asks — a bound on the order of vanishing at `-L` — is
shown to have been a real restriction by
`ft_weighted_dominance_one_admits_double_zero`. -/
theorem ft_weighted_dominance_one_hypotheses_nonvacuous :
  ...
  refine ⟨3, 3, ![1, 1, 1], 1, 1, by norm_num, ?_, by norm_num, by norm_num, ?_, ?_,
    by norm_num, by norm_num, hasRealCoeffs_one, by norm_num⟩
...
/-- **The dropped hypothesis was a real restriction, not bookkeeping.**  At every
collision point the corner can return, `(X + L)^2` is an admissible numerator —
real coefficients, nonvanishing at `0` — with `ord_{-L} = 2`, so the old
`ord_{-L}(B) ≤ 1` excluded it outright.

The manuscript excludes nothing of the kind: `lem:amplitude-divisor` gives
`p = \nu - (k-1)` at every `\nu`, and `k = 2` here.  Measured at
`Q = (1-t)^3` in `../scripts/check_upper_endpoint_multiplicity.py`, where the
amplitude at this numerator decays like `\eta` and no constant floor holds — which
is why the hypothesis could not simply be deleted. -/
theorem ft_weighted_dominance_one_admits_double_zero {L : ℝ} (hL : L ≠ 0) :
    HasRealCoeffs ((X + C ((L : ℝ) : ℂ)) ^ 2)
      ∧ ((X + C ((L : ℝ) : ℂ)) ^ 2).eval 0 ≠ 0
      ∧ ((X + C ((L : ℝ) : ℂ)) ^ 2).rootMultiplicity ((-L : ℝ) : ℂ) = 2 := by
  have hreal : HasRealCoeffs ((X + C ((L : ℝ) : ℂ)) ^ 2) := by
    rw [sq]
    exact (hasRealCoeffs_X.add (hasRealCoeffs_C_ofReal L)).mul
      (hasRealCoeffs_X.add (hasRealCoeffs_C_ofReal L))
  refine ⟨hreal, ?_, ?_⟩
  · simp only [eval_pow, eval_add, eval_X, eval_C, zero_add]
    exact pow_ne_zero 2 (by exact_mod_cast hL)
  · have hrw : (X + C ((L : ℝ) : ℂ)) = X - C ((-L : ℝ) : ℂ) := by
      rw [Complex.ofReal_neg, map_neg, sub_neg_eq_add]
    rw [hrw, Polynomial.rootMultiplicity_X_sub_C_pow]
