...
Formalizes `../shields-2026-forgacs-tran-numerators.tex`, «The elementary
linear case» (`subsec:linear-case`), `prop:linear-case`: the excluded elementary case
...
linear case, denominator pencil, real-rootedness
-/
...
/-- Paper `subsec:linear-case`, `prop:linear-case` — the linear-case coefficient
polynomial `P_m(z) = (-1)^m q₀^{-(m+1)} R(z) (z+q₁)^m`, as an element of `ℂ[X]`. -/
noncomputable def linCoeffPoly (R : ℂ[X]) (q0 q1 : ℝ) (m : ℕ) : ℂ[X] :=
  C ((-1) ^ m / (q0 : ℂ) ^ (m + 1)) * R * (X + C (q1 : ℂ)) ^ m

/-- **Paper `subsec:linear-case`, `prop:linear-case` — the generating relation.**
...
theorem linCoeffPoly_recurrence (R : ℂ[X]) {q0 : ℝ} (hq0 : q0 ≠ 0) (q1 : ℝ) (m : ℕ) :
    C (q0 : ℂ) * linCoeffPoly R q0 q1 (m + 1) + (X + C (q1 : ℂ)) * linCoeffPoly R q0 q1 m = 0 := by
...
/-- Paper `subsec:linear-case`, `prop:linear-case` (supporting) — `P_m ≠ 0` when
`R ≠ 0` and `q₀ ≠ 0`. -/
theorem linCoeffPoly_ne_zero {R : ℂ[X]} (hR : R ≠ 0) {q0 : ℝ} (hq0 : q0 ≠ 0) (q1 : ℝ) (m : ℕ) :
    linCoeffPoly R q0 q1 m ≠ 0 := by
  have hq0c : (q0 : ℂ) ≠ 0 := by exact_mod_cast hq0
  unfold linCoeffPoly
...
/-- Paper `subsec:linear-case`, `prop:linear-case` (supporting) — the root multiset of
`P_m` is that of `R` together with `m` copies of the moving zero `-q₁`. -/
theorem linCoeffPoly_roots {R : ℂ[X]} (hR : R ≠ 0) {q0 : ℝ} (hq0 : q0 ≠ 0) (q1 : ℝ) (m : ℕ) :
    (linCoeffPoly R q0 q1 m).roots = R.roots + m • ({-(q1 : ℂ)} : Multiset ℂ) := by
  have hq0c : (q0 : ℂ) ≠ 0 := by exact_mod_cast hq0
  ...
  change (C ((-1 : ℂ) ^ m / (q0 : ℂ) ^ (m + 1)) * R * (X + C (q1 : ℂ)) ^ m).roots = _
...
theorem linCoeffPoly_exceptional_eq {R : ℂ[X]} (hR : R ≠ 0) {q0 : ℝ} (hq0 : q0 ≠ 0)
    {q1 : ℝ} (hq1 : q1 < 0) (m : ℕ) :
    exceptionalRoots (linCoeffPoly R q0 q1 m) posRay = exceptionalRoots R posRay := by
  ...
  rw [linCoeffPoly_roots hR hq0 q1 m, Multiset.filter_add, hnil, add_zero]

open scoped Classical in
/-- **`prop:linear-case`, uniform bound.**  Every `P_m` has at most `deg R`
exceptional zeros — a bound independent of `m`. -/
theorem linCoeffPoly_exceptional_card_le {R : ℂ[X]} (hR : R ≠ 0) {q0 : ℝ} (hq0 : q0 ≠ 0)
    {q1 : ℝ} (hq1 : q1 < 0) (m : ℕ) :
    (exceptionalRoots (linCoeffPoly R q0 q1 m) posRay).card ≤ R.natDegree := by
  rw [linCoeffPoly_exceptional_eq hR hq0 hq1 m]
...
`linCoeffPoly` is a *defined* closed form.  What `prop:linear-case` asserts is that the
coefficient sequence of `R/(q₀+(q₁+z)t)` **equals** it, and that is a uniqueness
statement: the `sec:reduction` denominator recurrence plus the initial datum pins the sequence.
...
for `j ≥ 2` — the `ftDenom` of `sec:reduction` specialised to `deg Q = r = 1`. -/
...
theorem denomConv_dlin_linCoeffPoly (R : ℂ[X]) {q0 : ℝ} (hq0 : q0 ≠ 0) (q1 : ℝ) (m : ℕ) :
    denomConv (dlin q0 q1) (linCoeffPoly R q0 q1) m = if m = 0 then R else 0 := by
  cases m with
  | zero =>
      have hq0c : (q0 : ℂ) ≠ 0 := by exact_mod_cast hq0
  ...
  | succ n =>
      ...
      have := linCoeffPoly_recurrence R hq0 q1 n
...
denominator recurrence with numerator `R` *is* the closed form `linCoeffPoly`.  Together
with `linCoeffPoly_exceptional_card_le` this gives the surviving first conclusion of
`thm:main` for the excluded case `deg Q = r = 1`. -/
theorem linCoeffPoly_unique (R : ℂ[X]) {q0 : ℝ} (hq0 : q0 ≠ 0) (q1 : ℝ) (P : ℕ → ℂ[X])
    (h : ∀ m, denomConv (dlin q0 q1) P m = if m = 0 then R else 0) :
    P = linCoeffPoly R q0 q1 := by
  have hq0c : (q0 : ℂ) ≠ 0 := by exact_mod_cast hq0
  ...
  · intro m
    rw [h m, denomConv_dlin_linCoeffPoly R hq0 q1 m]
