...
The paper carries two figure floats: `fig:decomposition-and-defect`,
whose panels (A) and (B) sit under one caption, and `fig:clock-collapse`, a single panel.
...
three axis environments from the manuscript itself -- not from a hand-kept copy,
...
# One entry per figure float, in source order: its label and the expected
# \addplot count per panel.  A change here means a figure was edited and its
# caption claims need rechecking too.
FIGURES = (
    ("fig:decomposition-and-defect", (5, 8)),
    ("fig:clock-collapse", (4,)),
)
...
def extract_colors(paper_text):
    """The manuscript's \\definecolor lines, hoisted into the wrapper preamble.

    Extracting axis bodies alone leaves every color undefined and pdflatex
    dies, so they have to come across.  Extracted rather than restated here,
    for the same reason the axes are: a second copy would drift from the
    manuscript.  They are declared in the manuscript's own preamble -- a color
    declared inside a float is scoped to it, so each float would need its own
    copy -- and this scan reads the whole file, so it finds them wherever they
    sit.
    """
    found = {}
    for name, spec in re.findall(
        r"\\definecolor\{(\w+)\}\{(RGB\}\{[\d,\s]+)\}", paper_text
    ):
        prev = found.setdefault(name, spec)
        assert prev == spec, (
            f"color {name} is defined two ways in the manuscript: "
            f"{prev} and {spec}"
        )
    assert found, (
        "no \\definecolor found in the manuscript; the figures need "
        "marker colors -- check whether they were renamed or removed"
    )
    return "".join(
        f"\\definecolor{{{n}}}{{{spec}}}\n" for n, spec in sorted(found.items())
    )
...
def extract_panels(paper_text):
    ...
    out = []
    for label, expected in FIGURES:
        out.extend(extract_one(paper_text, label, expected))
    return out


def extract_one(paper_text, FIGURE_LABEL, EXPECTED_ADDPLOTS):
    ...
    for k, body in enumerate(panels):
        ...
        if k > 0:                      # only a sibling panel can carry placement
            assert stripped, (
    ...
    # the first panel must not depend on anything either
...
% shields-2026-forgacs-tran-numerators.tex, every figure float in source order.
...
def verify(n_panels):
    ...
    # per-page landmarks, so a page-order swap is caught
    p1, p2, p3 = (doc[k].get_text() for k in range(3))
    assert "(A)" in p1, "page 1 is not panel (A)"
    assert "(B)" in p2, "page 2 is not panel (B)"
    assert "40" in p1, "panel (A) y-tick labels missing"
    assert "both indices" in p2, "panel (B) callout missing"
    assert "z" in p2, "panel (B) limit-point labels missing"
    assert "14" in p2 and "38" in p2, "panel (B) legend entries missing"
    assert "16" in p3 and "28" in p3 and "44" in p3, (
        "page 3 is not the clock collapse: its three index legend entries are missing"
    )
    doc.close()
...
def main():
    ...
    print(f"extracted {len(panels)} panels from "
          + ", ".join(lbl for lbl, _ in FIGURES))

    colors = extract_colors(paper_text)
    print(f"hoisted {colors.count(chr(92) + chr(100))} color definitions "
          "from the figure floats")
    WRAPPER.write_text(build_wrapper(panels, colors), encoding="utf-8")
