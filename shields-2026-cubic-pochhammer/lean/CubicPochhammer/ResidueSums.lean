...
`shields-2026-cubic-pochhammer.tex`, `subsec:constant-weight-kernel` «The
constant-weight kernel», supporting the certificate sum `S_{n,j}` (`eq:S-def`,
`eq:S-table`).

Central object: `residueSum a j = ∑_{k ≡ a (3), k ≤ j} C(j,k)`, the count of size-`k`
...
(`residueSum_succ`), with the table's own recurrence (`corr_rec`) discharged by a finite
`decide` over the 3x6 residue grid.

First and second residue-class moments (`moment1_c2`, `moment_kk_c2`) are then
...
* `residueSum` --- `residueSum a j = ∑_{k ≤ j, k ≡ a (3)} C(j,k)`, over `ℤ`.
* `corrTab`, `corr` --- the period-6 correction table, and the correction term
  of the closed form.

## Main statements

* `residueSum_succ` --- Pascal's rule for `residueSum`.
* `three_residueSum_closed` --- the closed form
  `3 · residueSum a j = 2^j + corr a j`.
* `moment1_c2`, `moment_kk_c2` --- the first and second residue-class moments,
  which `Snj.lean` consumes.

## References

* `shields-2026-cubic-pochhammer.tex`, `subsec:constant-weight-kernel`
  «The constant-weight kernel»: `eq:S-def`, `eq:S-table`, `eq:S-R`.
-/
...
/-- `residueSum a j = ∑_{k ≤ j, k ≡ a (mod 3)} C(j,k)`, over `ℤ`.  The residue is taken
mod `3` internally, so `residueSum (a+2) j` automatically sums the shifted class. -/
def residueSum (a j : ℕ) : ℤ :=
  ∑ k ∈ Finset.range (j + 1), if k % 3 = a % 3 then (j.choose k : ℤ) else 0

/-- **Pascal recurrence** for the residue-class sums:
`residueSum a (j+1) = residueSum a j + residueSum (a+2) j`.  The second term collects the `C(j,k-1)`
half of `C(j+1,k) = C(j,k) + C(j,k-1)`, whose indices lie in the class `a-1 ≡
a+2 (mod 3)`. -/
theorem residueSum_succ (a j : ℕ) :
    residueSum a (j + 1) = residueSum a j + residueSum (a + 2) j := by
  unfold residueSum
  ...
  -- Sum1 = residueSum (a+2) j : rewrite predicate (k+1)%3 = a%3  ↔  k%3 = (a+2)%3
  have hSum1 : (∑ k ∈ Finset.range (j + 1),
    ...
    simp only [this]
  -- Sum2 + f0 = residueSum a j : reindex ∑ g(k+1) and reabsorb the endpoint
...
/-- The correction term of the closed form `3·residueSum a j = 2^j + corr a j`. -/
def corr (a j : ℕ) : ℤ := corrTab (a % 3) (j % 6)

/-- `corr` read off the table at a known residue: it depends on `a` only mod `3` and on
`j` only mod `6`, so pinning both residues evaluates it. -/
theorem corr_eq_corrTab {a j r : ℕ} (ha : a < 3) (hj : j % 6 = r) :
    corr a j = corrTab a r := by
  unfold corr; rw [hj, Nat.mod_eq_of_lt ha]

/-- The table satisfies the same Pascal step as `residueSum`:
`corr a (j+1) = corr a j + corr (a+2) j`.  Finite verification over the
`3 × 6` residue grid. -/
theorem corr_rec (a j : ℕ) : corr a (j + 1) = corr a j + corr (a + 2) j := by
  unfold corr
  have hjmod : (j + 1) % 6 = (j % 6 + 1) % 6 := by omega
  have hamod : (a + 2) % 3 = (a % 3 + 2) % 3 := by omega
  rw [hjmod, hamod]
...
`3 · residueSum a j = 2^j + corr a j`.  Single induction on `j`; the step is Pascal
(`residueSum_succ`) plus the table recurrence (`corr_rec`), and `2^j + 2^j = 2^{j+1}`. -/
theorem three_residueSum_closed (a j : ℕ) : 3 * residueSum a j = 2 ^ j + corr a j := by
  induction j generalizing a with
  | zero =>
    unfold residueSum corr
    rw [Finset.sum_range_one]
    have hp : a % 3 = 0 ∨ a % 3 = 1 ∨ a % 3 = 2 := by omega
    rcases hp with h | h | h <;> simp [h, corrTab]
  | succ j ih =>
    rw [residueSum_succ, mul_add, ih a, ih (a + 2), corr_rec, pow_succ]
    ring

/-- `residueSum` depends on its residue only mod `3`. -/
theorem residueSum_mod_eq {a b : ℕ} (j : ℕ) (h : a % 3 = b % 3) :
    residueSum a j = residueSum b j := by
  unfold residueSum; simp_rw [h]
...
theorem choose_mul1 (j k : ℕ) :
  ...
  have hshift : ((j + 1 : ℕ) * j.choose k : ℤ) = ((j + 1).choose (k + 1) * (k + 1 : ℕ) : ℤ) := by
    exact_mod_cast h
  push_cast at hshift ⊢
  linear_combination -hshift

/-- **First residue-class moment** (`∑ k·C = shifted residueSum`, `eq:S-R` input):
`∑_{k ≤ j+1} k·[k ≡ a] C(j+1,k) = (j+1)·residueSum (a+2) j`. -/
theorem moment1 (a j : ℕ) :
    (∑ k ∈ Finset.range (j + 2), (k : ℤ) * (if k % 3 = a % 3 then ((j + 1).choose k : ℤ) else 0))
      = ((j + 1 : ℕ) : ℤ) * residueSum (a + 2) j := by
  rw [Finset.sum_range_succ']
...
`∑_{k ≤ j} k·[k ≡ 2] C(j,k) = j·residueSum 1 (j-1)`. -/
theorem moment1_c2 (j : ℕ) :
    (∑ k ∈ Finset.range (j + 1), (k : ℤ) * (if k % 3 = 2 then (j.choose k : ℤ) else 0))
      = (j : ℤ) * residueSum 1 (j - 1) := by
  cases j with
  | zero => simp [residueSum]
  | succ j =>
    have h := moment1 2 j
    rw [residueSum_mod_eq j (show (2 + 2) % 3 = 1 % 3 by norm_num)] at h
    simpa using h
...
theorem choose_mul2 (j k : ℕ) :
  ...
  have hsucc := choose_mul1 (j + 1) (k + 1)
  have hbase := choose_mul1 j k
  push_cast at hsucc hbase ⊢
  linear_combination ((k : ℤ) + 1) * hsucc + ((j : ℤ) + 2) * hbase

/-- **Second residue-class moment** (`∑ k(k-1)·C = twice-shifted residueSum`):
`∑_{k ≤ j+2} k(k-1)·[k ≡ a] C(j+2,k) = (j+2)(j+1)·residueSum (a+1) j`. -/
theorem moment2 (a j : ℕ) :
    (∑ k ∈ Finset.range (j + 3), (k : ℤ) * ((k : ℤ) - 1) *
        (if k % 3 = a % 3 then ((j + 2).choose k : ℤ) else 0))
      = ((j + 2 : ℕ) : ℤ) * ((j + 1 : ℕ) : ℤ) * residueSum (a + 1) j := by
  ...
  have step : ∀ l ∈ Finset.range (j + 1),
    ...
    · rw [if_pos hP, if_pos (hpred.mp hP)]
      ...
      have hcast : (((l + 2 : ℕ) : ℤ) - 1) = ((l + 1 : ℕ) : ℤ) := by push_cast; ring
      rw [hcast]; linear_combination hc
    · rw [if_neg hP, if_neg (fun h => hP (hpred.mpr h))]; ring
  rw [Finset.sum_congr rfl step, ← Finset.mul_sum]
  rfl

/-- Second moment specialized to `k ≡ 2 (mod 3)` at top index `j`:
`∑_{k ≤ j} k(k-1)·[k ≡ 2] C(j,k) = j(j-1)·residueSum 0 (j-2)`. -/
theorem moment_kk_c2 (j : ℕ) :
    (∑ k ∈ Finset.range (j + 1), (k : ℤ) * ((k : ℤ) - 1) *
        (if k % 3 = 2 then (j.choose k : ℤ) else 0))
      = (j : ℤ) * ((j : ℤ) - 1) * residueSum 0 (j - 2) := by
  obtain _ | _ | i := j
  · simp [residueSum]
  · norm_num [Finset.sum_range_succ]
  · have h := moment2 2 i
    rw [residueSum_mod_eq i (show (2 + 1) % 3 = 0 % 3 by norm_num)] at h
